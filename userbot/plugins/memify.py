...
async def _(event):
    ...
    if reply_message.sender.bot:
       ...
       await event.edit("```Mwahahahah! Vediamo ti dare di ritocchini a questa foto! (」ﾟﾛﾟ)｣ ```")
    ...
    async with borg.conversation("@MemeAutobot") as bot_conv:
          ...
          if "Okay..." in response.text:
            await event.edit("```🤨 APOCADAUS?! Questa non è una foto! Ma io ho sempre un piano B 🧐```")
            thumb = None
            ...
            await borg.send_file(  # pylint:disable=E0602
                ...
                requires_file_name,
                supports_streaming=False,
                caption="Userbot: Powered by @AnonHexo",
                # Courtesy: @A_Dark_Princ3
            )
            await event.delete()
            sax = await borg.send_message(event.chat_id, "🌀🌀Dovresti ringraziare @AnonHexo 🔥🔥")
            await asyncio.sleep(4)
            sax.delete()
          elif not is_message_image(reply_message):
            await event.edit("Scegli un messaggio giusto cretino.")
            return
          else: 
